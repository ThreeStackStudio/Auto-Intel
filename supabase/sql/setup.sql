...
create index if not exists idx_edge_rate_limit_user_function_mode_created
  on public.edge_rate_limit_events (user_id, function_name, mode, created_at desc);

create index if not exists idx_edge_rate_limit_created_at
  on public.edge_rate_limit_events (created_at desc);
...
begin
  ...
  perform pg_advisory_xact_lock(hashtext(coalesce(p_function_name, '') || ':' || coalesce(p_mode, '') || ':' || p_user_id::text));
  ...
  if v_count >= p_max_requests then
    v_reset_at := coalesce(v_oldest_in_window + make_interval(secs => p_window_seconds), v_now + make_interval(secs => p_window_seconds));
  ...
  insert into public.edge_rate_limit_events (user_id, function_name, mode)
  values (p_user_id, p_function_name, p_mode);
...
revoke all on function public.consume_edge_rate_limit(uuid, text, text, integer, integer) from public;
grant execute on function public.consume_edge_rate_limit(uuid, text, text, integer, integer) to service_role;
